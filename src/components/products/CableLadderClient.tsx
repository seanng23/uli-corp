...
import {
  CABLE_LADDER_PROFILES,
  CABLE_LADDER_STANDARDS,
  CABLE_LADDER_LENGTHS,
  type LadderDim,
...
function DimTable({ dims }: { dims: LadderDim[] }) {
  return (
      ...
      <table className="w-full text-left">
        <thead className="sticky top-0">
          <tr className="bg-[#F0E6CC]">
            ...
            <th className="font-raleway text-[11px] font-bold uppercase tracking-wider text-[#1A0F00] px-3 py-2 border-b border-[#1A0F00]/20">Thickness (mm)</th>
          </tr>
        </thead>
        <tbody>
          {dims.map((row, i) => (
...
function AccessoryList({ items }: { items: string[] }) {
...
export default function CableLadderClient() {
  const { addToCart } = useCart();

  const [profileCode, setProfileCode] = useState("LZ");
  const profile = CABLE_LADDER_PROFILES.find((p) => p.code === profileCode) ?? CABLE_LADDER_PROFILES[0];
  ...
  function handleAddToCart() {
    ...
    const id = generateItemId(`cable-ladder-${profile.code.toLowerCase()}`, specs);
    addToCart({
      id,
      productName: `Cable Ladder (${profile.code})`,
  ...
  const lengthLabel = `${CABLE_LADDER_LENGTHS[0]}mm or ${CABLE_LADDER_LENGTHS[1]}mm`;
    ...
    <>
      ...
      <div className="site-container pt-5 pb-2">
        <nav className="flex items-center gap-2 font-raleway text-[12px] text-[#5C4A30]">
          ...
          <span>/</span>
          <span className="text-[#1A0F00] font-semibold">Cable Ladder</span>
        </nav>
      </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-[1fr_500px] xl:grid-cols-[1fr_540px] gap-10 lg:gap-14 items-start">
          ...
          <div>
                  ...
                  return (
                    <button
                      ...
                      className={`relative aspect-square w-full overflow-hidden rounded-lg border transition-colors cursor-pointer ${
                        isActive ? "border-[#ff8905] border-2" : "border-[#1A0F00]/20 hover:border-[#1A0F00]/40"
                      }`}
                    >
                      <Image src={img} alt={`Cable ladder thumbnail ${i + 1}`} fill className="object-cover object-center" sizes="104px" />
                    </button>
                  );
                })}
              </div>
              <div className="relative flex-1 aspect-square overflow-hidden rounded-2xl border border-[#1A0F00]/20">
                <Image src={galleryImages[activeImage]} alt={`U-LI Cable Ladder ${profile.code}`} fill priority className="object-cover object-center" sizes="(max-width:1024px) 100vw, 50vw" />
              </div>
            </div>
            ...
            <CollapsibleSection id="description" title="Description" defaultOpen>
              <p className="font-raleway text-[15px] text-[#5C4A30] leading-relaxed">
                U-LI steel cable ladders carry heavy power and cabling loads over long spans across
                commercial, industrial and infrastructure installations. Available in five profiles —
                LZ, LN, LG, LC and L6N — and certified to IEC 61537 (SIRIM QAS International) and
                NEMA VE-1, with preferred lengths of 3.0m and 3.7m and widths up to 1000mm.
              </p>
            </CollapsibleSection>
            ...
            <CollapsibleSection id="properties" title="Properties">
              <div className="space-y-5">
                <div>
                  <p className="font-raleway text-[12px] font-bold uppercase tracking-widest text-[#1A0F00] mb-2">Finishing Options</p>
                  <ul className="grid grid-cols-1 sm:grid-cols-2 gap-x-6 gap-y-1">
                    {FINISHING.map((f) => (
                ...
                <div>
                  <p className="font-raleway text-[12px] font-bold uppercase tracking-widest text-[#1A0F00] mb-2">Applicable Standards</p>
                  <ul className="space-y-1">
                    ...
                    <li className="font-raleway text-[14px] text-[#5C4A30] flex items-center gap-2">
                      <span className="w-1 h-1 rounded-full bg-[#ff8905] flex-shrink-0" />
                      NEMA VE-1 — NEMA 10A (3.0m) &amp; NEMA 12B (3.7m)
                    </li>
                  </ul>
                </div>
              </div>
            ...
            <CollapsibleSection id="dimensions" title="Available Dimensions">
              <p className="font-raleway text-[13px] text-[#5C4A30] mb-4">
                ...
                lengths 3.0m and 3.7m.
              </p>
              <div className="space-y-3">
                {CABLE_LADDER_PROFILES.map((p) => (
            ...
            <CollapsibleSection id="accessories" title="Accessories">
              ...
              <div className="space-y-3">
                {CABLE_LADDER_PROFILES.map((p) => (
                  <ProfileAccordion key={p.code} code={p.code} itemCode={p.itemCode} defaultOpen={false}>
                    <AccessoryList items={p.accessories} />
                  </ProfileAccordion>
                ))}
              </div>
            </CollapsibleSection>
          ...
          <div>
            <div className="border border-white/40 p-6 bg-white/15 rounded-2xl shadow-[0_8px_30px_rgba(26,15,0,0.12)]">
              <h1 className="font-typewriter text-[clamp(1.4rem,2vw,1.9rem)] leading-tight text-[#1A0F00] mb-4">Cable Ladder</h1>

              {/* Profile selector */}
              <div className="mb-5">
                <p className="font-raleway text-[11px] font-bold uppercase tracking-widest text-[#1A0F00] mb-3">Design Profile</p>
                <div className="grid grid-cols-5 gap-2">
                  {CABLE_LADDER_PROFILES.map((p) => (
                    <button
                      key={p.code}
                      onClick={() => selectProfile(p.code)}
                      className={`px-1 py-2 rounded-md font-raleway text-[12px] font-bold border transition-all duration-150 ${
                        profileCode === p.code ? "bg-[#ff8905] border-[#ff8905] text-white" : "bg-transparent border-[#1A0F00]/40 text-[#1A0F00] hover:border-[#1A0F00]"
                      }`}
                    >
                      {p.code}
                    </button>
              ...
              <div className="mb-5 pb-5 border-b border-[#1A0F00]/15">
                <div className="flex flex-wrap gap-x-4 gap-y-1">
                  ...
                  <span className="font-raleway text-[12px] text-[#5C4A30]">L: {lengthLabel}</span>
              ...
              {/* Standard */}
              <div className="mb-5">
                <p className="font-raleway text-[11px] font-bold uppercase tracking-widest text-[#1A0F00] mb-3">Standard</p>
                  ...
                  {STANDARDS.map((s) => (
                    <button
                      ...
                      className={`inline-flex items-center justify-center px-3 py-2 rounded-md font-raleway text-[11px] font-semibold leading-none tabular-nums border transition-all duration-150 ${
                        selectedStandard === s ? "bg-[#ff8905] border-[#ff8905] text-white" : "bg-transparent border-[#1A0F00]/40 text-[#1A0F00] hover:border-[#1A0F00]"
                      }`}
                    >
                      {s}
                    </button>
              ...
              <div className="mb-5 grid grid-cols-2 gap-3">
                {([
                  ...
                  { key: "Length", label: "Length (mm)", opts: CABLE_LADDER_LENGTHS.map(String) },
                  { key: "Thickness", label: "Thickness (mm)", opts: thicknesses.map((t) => t.toFixed(1)) },
                ] as const).map((field) => (
                  <div key={field.key}>
                    <label className="font-raleway text-[11px] font-bold uppercase tracking-widest text-[#1A0F00] block mb-1.5">{field.label}</label>
                    <DimensionCombobox
              ...
              {/* Finishing */}
              <div className="mb-5">
                <p className="font-raleway text-[11px] font-bold uppercase tracking-widest text-[#1A0F00] mb-3">Finishing</p>
                  ...
                  {FINISHING.map((f) => (
                    <button
                      ...
                      className={`px-3 py-2 rounded-md font-raleway text-[11px] font-semibold border transition-all duration-150 text-left leading-snug ${
                        selectedFinishing === f ? "bg-[#ff8905] border-[#ff8905] text-white" : "bg-transparent border-[#1A0F00]/30 text-[#1A0F00] hover:border-[#1A0F00]"
                      }`}
                    >
                      {f}
                    </button>
              ...
              {/* Finishing color */}
              <div className="mb-5">
                <p className="font-raleway text-[11px] font-bold uppercase tracking-widest text-[#1A0F00] mb-3">Finishing Color</p>
                <div className="flex flex-wrap gap-2">
                  {["Orange", "White", "Green", "Red", "Custom"].map((c) => {
                    const map: Record<string, string> = { orange: "#ff8905", white: "#ffffff", green: "#4a7c59", red: "#cc2222", custom: "#d9cba8" };
              ...
              {/* Quantity */}
              <div className="mb-6">
                <label className="font-raleway text-[11px] font-bold uppercase tracking-widest text-[#1A0F00] block mb-1.5">Quantity</label>
            ...
            {/* Table of Contents */}
            <div className="mt-8">
              <p className="font-raleway text-[11px] font-bold uppercase tracking-widest text-[#5C4A30] mb-3">Table of Contents</p>
