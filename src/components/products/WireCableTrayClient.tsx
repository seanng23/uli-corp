  ...
  return <>
    ...
    <div className="site-container py-10 lg:py-12"><div className="grid grid-cols-1 lg:grid-cols-[1fr_500px] xl:grid-cols-[1fr_540px] gap-10 lg:gap-14 items-start">
      <div className="contents lg:block">
        <ProductGallery images={galleryImages} alt="U-LI Wire Mesh Tray" active={activeImage} onChange={setActiveImage} className="order-1 lg:order-none lg:mb-10" />
        ...
        <CollapsibleSection id="dimensions" title="Dimensions"><div className="space-y-6">{TRAY_PROFILES.map((item) => <div key={item.height}><p className="font-raleway text-[12px] font-bold uppercase tracking-widest text-[#1A0F00] mb-3">FWB {item.height}</p><div className="overflow-x-auto"><table className="w-full text-left min-w-[620px] border border-[#1A0F00]/20"><thead><tr className="bg-[#F0E6CC]">{["Reference", "Height (mm)", "Width (mm)", "Length (mm)"].map((heading) => <th key={heading} className={thClass}>{heading}</th>)}</tr></thead><tbody>{item.rows.map((tray, index) => <tr key={tray.refSuffix} className={index % 2 === 1 ? "bg-[#1A0F00]/[0.03]" : ""}><td className={tdClass}>FWB{item.height} - {tray.refSuffix}</td><td className={tdClass}>{item.height}</td><td className={tdClass}>{tray.width}</td><td className={tdClass}>{tray.length}</td></tr>)}</tbody></table></div></div>)}</div></CollapsibleSection>
      ...
      <div className="order-2 lg:order-none"><div className="border border-white/40 p-6 bg-white/15 rounded-2xl shadow-[0_8px_30px_rgba(26,15,0,0.12)]">
